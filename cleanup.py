...
def cleanup_wav(wav_path):
    logging.info(
        f"Called method: cleanup_wav() curThd={threading.current_thread().name}  Cleaning path={wav_path}")
    """
    ...
    logging.info(
        f"Finishing method: cleanup_wav() curThd={threading.current_thread().name}")
